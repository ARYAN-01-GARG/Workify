...
export const verifyOTP = createAsyncThunk(
    ...
    async ({ contact, otp }:{ contact : string , otp : string}, { rejectWithValue, dispatch }) => {
        ...
        try {
            const response = await axios.post('https://naitikjain.me/api/v1/auth/validate', {
                contact,
                otp
            });
            toast.dismiss();
...
export const verifyForgotOTP = createAsyncThunk(
    ...
    async ({ contact, otp }:{ contact : string , otp : string}, { rejectWithValue, dispatch }) => {
        ...
        dispatch(setIsLoading(true));
        try {
            const response = await axios.post('https://naitikjain.me/api/v1/auth/verify-otp-forgotpassword', {
                contact,
                otp
            });
            toast.dismiss();
